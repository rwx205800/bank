...
public class EncryUtil {
    ...
    public static boolean checkDecryptAndSign(String data, String encrypt_key,
        ...
        if (jsonObject.get("cardlist") != null) {
            JSONArray array = jsonObject.getJSONArray("cardlist");
            if (array != null && array.size() > 0) {
                StringBuffer buffer = new StringBuffer();
                for (int i = 0; i < array.size(); i++) {
                    TreeMap<String, Object> mapArr = JSON.parseObject(array.getJSONObject(i).toJSONString(),
                            new TypeReference<TreeMap<String, Object>>() {
                            });
                    buffer.append(JSONArray.toJSONString(mapArr));
                    buffer.append(",");
                }
                String value = buffer.toString();
                map.replace("cardlist", "[" + value.substring(0, value.length() - 1) + "]");
            }
        }
