...
public class IdcardUtils extends StringUtils {
     ...
     * @return 年龄
     */
    public static int getAgeDecimalByIdCard(String idCard) {
        Date date = new Date();
        ...
        int d2 = Integer.parseInt(year);
        int mm = d1 - d2;
        String s = Integer.toString(mm);
        String y = s.substring(0, 2);
        String m = s.substring(3, 4);
        int fullyear = Integer.parseInt(y);
        int mon = Integer.parseInt(m);
        if (mon < 6)
            fullyear = fullyear - 1;
        //System.out.println("我的周岁是："+fullyear);
        Calendar cal = Calendar.getInstance();
        int iCurrYear = cal.get(Calendar.YEAR);//得到年
    ...
    public static void main(String[] args) {
        int age = getAgeDecimalByIdCard("342422198908105811");
     ...
     * @return 性别(M-男，F-女，N-未知)
     */
    public static String getGenderByIdCardChina(String idCard) {
        String sGender = "";
        try {
            if (idCard.length() == CHINA_ID_MIN_LENGTH) {
                idCard = conver15CardTo18(idCard);
     ...
     * @return 提取的数字。
     */
    public static String getRandam15() {
